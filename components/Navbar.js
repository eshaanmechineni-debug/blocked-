      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center cursor-pointer group" onClick=${onLogoClick}>
            ...
            <span className="text-xl font-extrabold tracking-tighter text-white">NEXUS<span className="text-indigo-400">GAMES</span></span>
          </div>
          ...
          <div className="flex-1 max-w-md mx-8 hidden sm:block">
            <input
              ...
              className="block w-full bg-slate-800 border-none rounded-full py-2 px-4 text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-white"
              onChange=${(e) => onSearch(e.target.value)}
            />
          </div>

          <div className="flex items-center space-x-4">
            <button className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg text-sm font-bold transition-all">Join Discord</button>
          </div>
        </div>
      </div>
    </nav>
